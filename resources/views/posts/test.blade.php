    ...
    <div class="col-6 ">
        ...
        {{-- <div class="container" style="background-color:rgb(12, 11, 11) "> --}}
            <form action="{{route('home.posts.store')}}" method="POST" enctype="multipart/form-data" id="post_form">
                ...
                <button type="button" class="p-0 m-0 text-center" data-bs-toggle="modal" data-bs-target="#postFiles"  width="100%" height="100%">
                    Add Files
                </button>
                <div class="modal fade text-start" id="postFiles" tabindex="-1" aria-labelledby="postFiles" aria-hidden="true">
            ...
            <div class="row mx-3 justify-content-start" style='background-color:rgb(12, 11, 11)' ><!-- //one post -->
                <div class="col-2 my-3 "><!-- photo -->
                    <img src="{{asset('/photo/ahmad.jpg')}}" class="rounded float-start" alt="..." width="100%" height="100%">
                </div>
                ...
                <div class="col-1 my-3 py-4 "><!-- dropdown -->
                    <div class="dropdown">
                        <a class="btn btn-secondary m-0 p-0 dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                        </a>
                ...
                <div class="col-3">
                     <a href="" class="p-0 m-0 text-center"  width="100%" height="100%"><p class="h5 mx-2" style="display: inline-block">{{ count($posts[0]->like)}}</p><p class="h5" style="display: inline-block">likes</p></a>
                    <button type="button" class="p-0 m-0 text-center" data-bs-toggle="modal" data-bs-target="#exampleModal{{$post->id}}"  width="100%" height="100%">
                        likes
                    </button>
                    <div class="modal fade text-start" id="exampleModal{{$post->id}}" tabindex="-1" aria-labelledby="exampleModalLabel{{$post->id}}" aria-hidden="true">
                        <div class="modal-dialog ">
                            <div class="modal-content text-bg-dark">
                                <div class="modal-header">
                                ...
                                <div class="modal-body text-bg-dark">
                                    <table class="table table-dark table-striped">
                                        ...
                                        <tr class="row">
                                            <td class="col-2"><img src="{{ asset($like->user->photo->path) }}" alt="personal photo" class="rounded float-start"  width="100%" height="100%"></td>
                                            <td class="col-8"><a href="{{route('users.show' , $like->user )}}">{{$like->user->name}}</a></td>
                                            @if(!Auth::user()->id == $like->user->id)
                                                <td class="col-2">
                                                    @if(Auth::user()->follow->contains('user_follower',$like->user->id))
                                                        <form action="{{route('follows.destroy' , Auth::user()->follow->where('user_follower',$like->user->id) )}}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                             <input type="text" name="name" value="{{$like->user->name}}" style="display: none">
                ...
                <div class="col-3">
                    <a href="" class="p-0 m-0 text-center" width="100%" height="100%"><p class="h5 mx-1" style="display: inline-block">{{ count($post->comment)}}</p><p class="h5 mx-1" style="display: inline-block">comments</p></a>
                </div>
